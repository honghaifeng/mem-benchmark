...
DATA_PATH = os.path.join(BASE_DIR, "clong_data", "small.jsonl")
OUTPUT_DIR = os.path.join(BASE_DIR, "results", "mem0_official_clong70")
SCRIPT = os.path.join(BASE_DIR, "mem0_official_clong_eval.py")
NUM_CONV = 70
TOP_K = 30
...
def run_single_conv(conv_idx: int) -> dict:
    ...
    # 每个对话使用独立的 MEM0_DIR，避免 Qdrant 本地存储锁冲突
    mem0_dir = os.path.join(conv_output, "mem0_home")
    os.makedirs(mem0_dir, exist_ok=True)
    env["MEM0_DIR"] = mem0_dir

    cmd = [
        sys.executable, "-u",
        SCRIPT,
        "--data-path", DATA_PATH,
        "--conv-idx", str(conv_idx),
        "--top-k", str(TOP_K),
...
def main():
    ...
    print(f"Mem0 官方版 CLongEval 中文并行测评")
    ...
    for r in results:
        if r and r["status"] == "ok":
            ...
            total_memories += d.get("memory_count", 0)
            write_times.append(d.get("write_time_seconds", 0))
            answer_times.append(d.get("answer_time_seconds", 0))
    ...
    merged = {
        ...
        "write_time_seconds": sum(write_times),
        "answer_time_seconds": sum(answer_times),
        "total_time_seconds": total_elapsed,
    }
    merged_path = os.path.join(OUTPUT_DIR, "mem0_official_clong_merged.json")
